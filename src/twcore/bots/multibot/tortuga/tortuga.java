...
public class tortuga extends MultiModule {
    ...
    public void fatal(String errMsg) {
        ...
        if (m_host != "") {
            m_botAction.sendPrivateMessage(m_host, "FATAL: " + errMsg);
        }
    ...
    public void logError(String errMsg) {
        ...
        if (m_host != "") {
            m_botAction.sendPrivateMessage(m_host, "ERROR: " + errMsg);
        }
    }
    ...
    public void logDebug(String errMsg) {
        ...
        if (m_host != "") {
            m_botAction.sendPrivateMessage(m_host, "DEBUG: " + errMsg);
        }
    }
    ...
    public void cmdOn(String name, String msg) {
        ...
        if (isRunning) {
            m_botAction.sendPrivateMessage(name, "Tortuga is currently activated.  Use !shutdown to shut it down.");
            return;
        }
    ...
    public void cmdShutdown(String name, String msg) {
        ...
        if (!isRunning) {
            m_botAction.sendPrivateMessage(name, "Tortuga is not activated.");
            return;
        }
    ...
    public void mergeData() {
        try {
            ...
            if (r != null) {
                while (r.next()) {
                    ...
                    try {
                        ...
                        fatal("Error updating playerID " + playerID + " with " + newCoins + " coins: " + e.getMessage());
                    }

                    try {
                        m_botAction.SQLQueryAndClose(m_database, "UPDATE tblPirateStats set fnMerged = 1 where fnEntryID = " + entryID);
                    } catch (SQLException e) {
                        fatal("Error updating fnMerged to 1 for entryID " + entryID + ": " + e.getMessage());
                    }
                }
            }
            ...
            fatal("Error reading player stats [ERR: " + e.getMessage() + "]");
        }
    }
    ...
    public void cmdTop(String name, String msg) {
        if (!isRunning) {
            m_botAction.sendPrivateMessage(name, "Scores are not kept while Tortuga is shutdown.");
            return;
        }
        ...
        for (ScoreSet ss : m_scores.values()) {
            if (ss.freq0Kills > most_freq0Kills) {
                most_freq0Kills = ss.freq0Kills;
            }

            if (ss.freq1Kills > most_freq1Kills) {
                most_freq1Kills = ss.freq1Kills;
            }

            if (ss.kills > most_kills) {
                most_kills = ss.kills;
            }

            if (ss.flagTouches > most_flagTouches) {
                most_flagTouches = ss.flagTouches;
            }

            if (ss.flagCaptures > most_flagCaptures) {
                most_flagCaptures = ss.flagCaptures;
            }

            if (ss.foundCoins > most_foundCoins) {
                most_foundCoins = ss.foundCoins;
            }

            if (ss.bountyCoins > most_bountyCoins) {
                most_bountyCoins = ss.bountyCoins;
            }

            if (ss.taxCoins > most_taxCoins) {
                most_taxCoins = ss.taxCoins;
            }

            if (ss.deaths > most_deaths) {
                most_deaths = ss.deaths;
            }

            if (ss.deaths > 0) {
                if (roundto((ss.freq0Kills + ss.freq1Kills) / ss.deaths, 2) > best_ratio) {
                    best_ratio = curRatio;
                }
            }
        }
        ...
        if (!m_freqscores.isEmpty()) {
            ...
            if (m_freqscores.get("freq0FlagTime") > m_freqscores.get("freq1FlagTime")) {
                wfreq = "freq 0";
            }
        ...
        for (String strPlayer : m_scores.keySet()) {
            if (most_freq0Kills > 0 && m_scores.get(strPlayer).freq0Kills == most_freq0Kills) {
                wfreq0Kills += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_freq1Kills > 0 && m_scores.get(strPlayer).freq1Kills == most_freq1Kills) {
                wfreq1Kills += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_kills > 0 && m_scores.get(strPlayer).kills == most_kills) {
                wkills += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_flagTouches > 0 && m_scores.get(strPlayer).flagTouches == most_flagTouches) {
                wflagTouches += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_flagCaptures > 0 && m_scores.get(strPlayer).flagCaptures == most_flagCaptures) {
                wflagCaptures += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_foundCoins > 0 && m_scores.get(strPlayer).foundCoins == most_foundCoins) {
                wfoundCoins += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_bountyCoins > 0 && m_scores.get(strPlayer).bountyCoins == most_bountyCoins) {
                wbountyCoins += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_taxCoins > 0 && m_scores.get(strPlayer).taxCoins == most_taxCoins) {
                wtaxCoins += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (most_deaths > 0 && m_scores.get(strPlayer).deaths == most_deaths) {
                wdeaths += ", " + m_players.get(strPlayer).getPirateName();
            }

            if (best_ratio > 0.0f && m_scores.get(strPlayer).deaths > 0) {
                if (roundto((m_scores.get(strPlayer).freq0Kills + m_scores.get(strPlayer).freq1Kills) / m_scores.get(strPlayer).deaths, 2) == best_ratio) {
                    wratio += ", " + strPlayer;
                }
            }
        }
        ...
        if (wfreq0Kills.length() > 0) {
            m_botAction.sendArenaMessage(  "MOST KILLS (even freq) : " + rPadInt(most_freq0Kills, 6)         + "  " + wfreq0Kills.substring(2));
        }

        if (wfreq1Kills.length() > 0) {
            m_botAction.sendArenaMessage(  "MOST KILLS (odd freq)  : " + rPadInt(most_freq1Kills, 6)         + "  " + wfreq1Kills.substring(2));
        }

        if (wkills.length() > 0) {
            m_botAction.sendArenaMessage(       "MOST KILLS (overall)   : " + rPadInt(most_kills, 6)              + "  " + wkills.substring(2));
        }

        if (wflagTouches.length() > 0) {
            m_botAction.sendArenaMessage( "MOST FLAG TOUCHES      : " + rPadInt(most_flagTouches, 6)        + "  " + wflagTouches.substring(2));
        }

        if (wflagCaptures.length() > 0) {
            m_botAction.sendArenaMessage("MOST FLAG CAPTURES     : " + rPadInt(most_flagCaptures, 6)       + "  " + wflagCaptures.substring(2));
        }

        if (wfoundCoins.length() > 0) {
            m_botAction.sendArenaMessage(  "MOST COINS FOUND       : " + rPadInt(most_foundCoins, 6)         + "  " + wfoundCoins.substring(2));
        }

        if (wbountyCoins.length() > 0) {
            m_botAction.sendArenaMessage( "MOST BOUNTY EARNED     : " + rPadInt(most_bountyCoins, 6)        + "  " + wbountyCoins.substring(2));
        }

        if (wtaxCoins.length() > 0) {
            m_botAction.sendArenaMessage(    "MOST TAX COLLECTED     : " + rPadInt(most_taxCoins, 6)           + "  " + wtaxCoins.substring(2));
        }

        if (wdeaths.length() > 0) {
            m_botAction.sendArenaMessage(      "MOST DEATHS            : " + rPadInt(most_deaths, 6)             + "  " + wdeaths.substring(2));
        }

        if (wratio.length() > 0) {
            m_botAction.sendArenaMessage(       "BEST RATIO             : " + Tools.rightString("" + best_ratio, 6) + "  " + wratio.substring(2));
        }
    ...
    public void cmdRecords(String name, String msg) {
        if (!isRunning) {
            m_botAction.sendPrivateMessage(name, "Tortuga must be activated to use this command.");
            return;
        }
    ...
    public void cmdListHouses(String name, String msg) {
        ...
        Comparator<House> compHouse = new Comparator<House>() {
            public int compare(House a, House b) {
                ...
                else if (a.getHouseID() < b.getHouseID()) {
                    return -1;
                }
                else {
                    return 1;
    ...
    public void cmdStatus(String name, String msg) {
        ...
        if (cp.getHouseCount() > 0) {
            strHouses = " (" + cp.getHouseList() + ") ";
        }

        if (cp.getPlotCount() > 0) {
            strPlots = " (" + cp.getPlotList() + ") ";
        }
    ...
    public void cmdAward(String name, String msg) {
        ...
        if (params.length != 3 && params.length != 4) {
            m_botAction.sendPrivateMessage(name, "Huh?  You need to use !award <player>:<amount>:<reason>");
            return;
        }
        ...
        try {
            awAmount = Integer.parseInt(params[1]);
        }
    ...
    public void cmdBuyHouse(String name, String msg) {
        ...
        if (msgs.length != 2) {
            m_botAction.sendPrivateMessage(name, "Huh?  You need to use !buyhouse <house#> <price>");
            return;
        }
        ...
        try {
            curHouseID = Integer.parseInt(msgs[0]);
        }
        catch (NumberFormatException e) {}
        ...
        if (curHouseID > 100 && curHouseID < 155) {
            ...
            try {
                curOffer = Integer.parseInt(msgs[1]);
            }
            catch (NumberFormatException e) {}
            ...
            if (curOffer >= curPrice) {
                ...
                if (p.getCoins() >= curOffer) {
                    ...
                    if (curOwner != null) {
                        ...
                        if (m_botAction.getPlayer(curOwner) != null) {
                            m_botAction.sendPrivateMessage(curOwner, "Your house at " + curHouseID + " was purchased by " + name + " for " + curOffer + " coins.  " + curHouse.getCoinsPaid() + " coins have been returned to you.");
                        }
                    }
    ...
    public void cmdBuy(String name, String msg) {
        ...
        if (msgs.length != 2) {
            m_botAction.sendPrivateMessage(name, "Huh?  You need to use !buy <plot> <price>.");
            return;
        }
            ...
            if (curOffer >= curPrice) {
                ...
                if (p.getCoins() >= curOffer) {
                    ...
                    if (curOwner != null) {
                        ...
                        if (m_botAction.getPlayer(curOwner) != null) {
                            m_botAction.sendPrivateMessage(curOwner, "Your plot (" + curPlot.getAddr() + ") was purchased by " + name + " for " + curOffer + " coins.  " + curPlot.getCoinsPaid() + " coins have been returned to you.");
                        }
                    }
    ...
    public void cmdHouseInfo(String name, String msg) {
        ...
        if (msg.length() > 0) {
            try {
                curHouseID = Integer.parseInt(msg);
            }
            catch (NumberFormatException e) {}
        }
    ...
    public void loadPlots() {
        try {
            ...
            if (r == null) {
                fatal("Error reading plots: NULL resultset returned.");
            }
            ...
            fatal("Error reading plots [ERR: " + e.getMessage() + "]");
        }
    }
    ...
    public void loadHouses() {
        try {
            ...
            if (r == null) {
                fatal("Error reading houses: NULL resultset returned.");
            }
            ...
            fatal("Error reading houses [ERR: " + e.getMessage() + "]");
        }
    }
    ...
    public void cmdGuide(String name, String msg) {
        if (!boolInTortuga) {
            m_botAction.sendPrivateMessage(name, "You (and the bot) must be in ?go tortuga to use this command.");
            return;
        }
    ...
    */
    public PlotPoint getPrimaryPlot(int x, int y) {
        ...
        else if ((x >= 5 && x <= 18) && y == 0) {
            newx = 5;    // NWALL (north wall)
            newy = 0;
        }
        else if (x == 0 && (y >= 4 && y <= 17)) {
            newx = 0;    // WWALL (west wall)
            newy = 4;
        }
        else if ((x >= 1 && x <= 11) && y == 17) {
            newx = 1;    // SWALL (south wall)
            newy = 17;
        }
        else if (x == 19 && (y >= 1 && y <= 8)) {
            newx = 19;    // EWALL (east wall)
            newy = 0;
        }
        else if (x == 12 && (y >= 8 && y <= 17)) {
            newx = 19;    // EWALL (east wall)
            newy = 0;
        }
        ...
        else if ((x >= 0 && x <= 11) && (y == 18 || y == 19)) {
            newx = -1;    // NOFLY ZONE (SEA)
            newy = -1;
        }
        else if (x == 19 && (y >= 9 && y <= 19)) {
            newx = -1;    // NOFLY ZONE (SEA)
            newy = -1;
        }
    ...
    */
    ...
    public int loadOrCreatePlayer(String playerName, Boolean createPlayer) {
        ...
        try {
            ...
            if (r == null) {
                fatal("Error reading player info: NULL resultset returned.");
            }
            ...
            if (r.next()) {
                ...
                while (rp.next()) {
                    arrPlots.add(rp.getInt("fnPlotID"));
                }
                ...
                while (rp3.next()) {
                    arrHouses.add(rp3.getInt("fnHouseID"));
                }
                ...
                try {
                    ...
                    fatal("Error creating player: " + playerName + " - " + e.getMessage());
                }
            ...
            fatal("Error reading player info [ERR: " + e.getMessage() + "]");
        }
    ...
    public String generatePirateName(String playerName) {
        ...
        try {
            ...
            if (rp == null) {
                fatal("Error reading tblPirateNamesDB: NULL resultset returned.");
            }
            ...
            if (nameIsLower) {
                pirateName = pirateName.toLowerCase();
            }
        } catch (SQLException e) {
            fatal("Error selecting from tblPirateNamesDB: " + e.getMessage());
        }
    ...
    public int createPlayer(String playerName, String pirateName) {
        ...
        try {
            ...
            if (r == null) {
                fatal("Error inserting player: NULL resultset returned.");
            }
            ...
            fatal("Error creating player: " + playerName);
        }
    ...
    public int createGame() {
        ...
        try {
            ...
            if (r == null) {
                fatal("Error inserting game: NULL resultset returned.");
            }
            ...
            fatal("Error creating game: " + e.getMessage());
        }
    ...
    private class House {
        ...
        private String getOwnerPirateName() {
            return PirateName;
        }
        private int getCoinsPaid() {
            return CoinsPaid;
        }
        private int getPrice() {
            if (CoinsPaid == 0) {
                return HOUSEPRICE_DEFAULT;
            }
            else {
                return (int)(CoinsPaid + (CoinsPaid * HOUSEPRICE_INCREMENT));
            }
        }
        private boolean saveToDB() {
            if (hasChanged) {
                try {
                    ...
                    fatal("Error updating house: " + e.getMessage());
                }
        ...
        private void setCoinsPaid(int CoinsPaid) {
            this.CoinsPaid = CoinsPaid;
            hasChanged = true;
        }
        ...
        private void saveToDB() {
            if (isNew) {
                try {
                    ...
                    fatal("Error updating bank record: " + e.getMessage());
                }
        ...
        private void saveToDB() {
            if (hasChanged) {
                try {
                    ...
                    fatal("Error updating plot: " + e.getMessage());
                }
    ...
    private class Plot {
        ...
        private int getPrice() {
            if (CoinsPaid == 0) {
                ...
                return (int)(CoinsPaid + (CoinsPaid * PRICE_INCREMENT));
            }
        }
        private boolean saveToDB() {
            if (hasChanged) {
                ...
                if (PlayerName != null) {
                    playerID = m_players.get(PlayerName).getPlayerID();
                }
        ...
        private String getCaption() {
            if (m_specials.get(Addr) != null) {
                return m_specials.get(Addr);
            }
            else {
                return Addr;
            }
    ...
    private class Pirate {
        ...
        private void addRecord(BankRecord m_record) {
            m_bankrecords.add(m_record);
        }
        private int getPlayerID() {
            return PlayerID;
        }
        private String getPirateName() {
            return PirateName;
        }
        private void setPirateName(String PirateName) {
            this.PirateName = PirateName;
            pirateNameChanged = true;
        }
        private int getCoins() {
            return (origCoins + foundCoins + bountyCoins + taxCoins + otherCoins);
        }
        private int getFoundCoins() {
            return foundCoins;
        }
        private int getBountyCoins() {
            return bountyCoins;
        }
        private int getTaxCoins() {
            return taxCoins;
        }
        private List<Integer> getPlots() {
            return arrPlots;
        }
        private List<Integer> getHouses() {
            return arrHouses;
        }
        private Vector<BankRecord> getRecords() {
            return m_bankrecords;
        }
        private void addPlot(int plotID) {
            ...
            if (m_earnings.get(plotID) == null) {
                try {
                    ...
                    fatal("Error inserting plot earnings for player: " + PlayerID + ", plot: " + plotID + ".  Error: " + e.getMessage());
                }

                m_earnings.put(plotID, new PiratePlotEarnings(PlayerID, plotID, 0));
            }
        }
        private void removePlot(int plotID) {
            arrPlots.remove(arrPlots.indexOf(plotID));
        }
        private int getPlotCount() {
            return arrPlots.size();
        }
        ...
        private void addHouse(int houseID) {
            arrHouses.add(houseID);
        }
        private void removeHouse(int houseID) {
            arrHouses.remove(arrHouses.indexOf(houseID));
        }
        private int getHouseCount() {
            return arrHouses.size();
        }
        ...
        private void saveToDB() {
            if (pirateNameChanged) {
                try {
                    ...
                    fatal("Error updating player: " + e.getMessage());
                }
            }
            ...
            if (earningsChanged) {
                for (PiratePlotEarnings p : m_earnings.values()) {
                    p.saveToDB();
                }
            ...
            try {
                ...
                fatal("Error updating player stats: " + e.getMessage());
            }
        }
    }
}
