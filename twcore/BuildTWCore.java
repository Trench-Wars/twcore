...
public class BuildTWCore {
    ...
    // Command line flags
    boolean bbuildAllCore = false;  // True to build the entire core
    ...
    public void handleArguments(String[] args) {
        ...
        for (int i=0; i<args.length; i++) {
            ...
            if (s.equals("core")) bbuildAllCore = true;
            else if (s.startsWith("core")) coreList.add("twcore/" + s);
                ...
                System.out.println("  build core/<dirname>   (builds only that directory within core)");
                ...
                System.out.println("build matchbot pubbot");
                System.out.println("build core");
                System.out.println("build core/stats core/sql");
     ...
     * @param sourcepath
     */
    public void recursiveCompile(File f, boolean dig, boolean bot, String extrapath) {
        if (f.isDirectory() && !f.getName().endsWith("CVS") ) {
    ...
    public void buildCore() {
        try {
            ...
            String classpath = current.getAbsolutePath();
            ...
            // Update the .jar with the additions
            if (doneSomething) {
                	bldCmd = binDir + "jar";
                fullExec(new String[] {bldCmd, "cf", "twcore.jar", "-C", "temp", "."});
            }
    ...
    public void buildBots() {
    	...
    	if(bbuildAllBots)
    		recursiveCompile(new File("twcore/bots"), true, true, new File("").getAbsolutePath());
    	else if(botList.size() > 0)
    	{
    		for (int i=0; i < botList.size(); i++)
    		{
    			recursiveCompile(new File("twcore/bots/"+botList.get(i)), true, true, new File("").getAbsolutePath());
    		}
    	}
        ...
        for (int i=0; i < botDirs.length; i++) {
            if ((bbuildAllBots) || (botList.contains(botDirs[i].getName()))) {
                try {
					...
					if (hasContent)
                        recursiveCompile(botDirs[i], true, true, new File("").getAbsolutePath());
                } catch (Exception e) {
