...
public class TwlMatch {
	...
	public TwlMatch( ResultSet result ) {
		try {
			...
			if( matchTypeId < 3 ) teamLimit = 5;
			else teamLimit = 8;
			if( matchTypeId < 3 ) deathLimit = 10;
	...
	public boolean state() { return stateSaved; }
	public void toggleState() { stateSaved = !stateSaved; }
	...
	public String getMatchType() {
		if( matchTypeId == 1 ) return "TWLD";
		else if( matchTypeId == 2 ) return "TWLJ";
		else if( matchTypeId == 3 ) return "TWLB";
		else return "Game";
	}
	...
	public void addSub( int id ) {
		if( id == team1Id ) team1Subs++;
		else if( id == team2Id ) team2Subs++;
	}
	
	public boolean subLimitReached( int id ) {
		int subLimit = 5;
		if( matchTypeId < 3 ) subLimit = 2;
		if( id == team1Id && team1Subs >= subLimit ) return true;
		else if( id == team2Id && team2Subs >= subLimit ) return true;
		else return false;
	}
	
	public boolean lagoutLimitReached( String player ) {
		int lagouts = getPlayer( player ).getData( 1 );
		if( matchTypeId < 3 && lagouts >= 3 ) return true;
		if( lagouts >= 5 ) return true;
		return false;
	} 
	...
	public void addSwap( int id ) {
		if( id == team1Id ) team1Swaps++;
		else if( id == team2Id ) team2Swaps++;
	}
	
	public boolean swapLimitReached( int id ) {
		int swapLimit = 3;
		if( matchTypeId < 3 ) swapLimit = 0;
		if( id == team1Id && team1Swaps >= swapLimit ) return true;
		else if( id == team2Id && team2Swaps >= swapLimit ) return true;
		else return false;
	}
	...
	public boolean isTeamMember( int id ) {
		if( id == team1Id || id == team2Id ) return true;
		else return false;
	}
	
	public boolean isTeamOne( int id ) {
		if( id == team1Id ) return true;
		else return false;
	}
	
	public boolean isTeamTwo( int id ) {
		if( id == team2Id ) return true;
		else return false;
	}
	
	public boolean rosterLimit( int id ) {
		if( team1Id == id )
			if( team1Size >= teamLimit ) return true;
			else return false;
		else if( team2Id == id )
			if( team2Size >= teamLimit ) return true;
			else return false;
		else return true;
	}
	...
	public boolean alreadyHasPlayer( String player ) {
		if( team1.containsKey( player ) || team2.containsKey( player ) ) return true;
		else return false;
	}
	
	public void zoned() { zoned = true; }
	public boolean haveZoned() { return zoned; }
	...
	public boolean gameOver( BotAction m_botAction ) {
    	...
    	while( it.hasNext() ) {
    		...
    		String player = (String) it.next();
			if( getPlayer( player ).getData( 9 ) == -1 ) playerOut = true;
			if( getPlayer( player ).getData( 1 ) >= 3 ) playerOut = true;  //Checks for 3 lagouts.
			int time = (int)(System.currentTimeMillis() / 1000);
			if( getPlayer( player ).getData(-9) == -2 ) playerOut = true;  //Checks for lagged out too long.
