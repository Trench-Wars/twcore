		...
		try {
	        ...
	        while(it.hasNext())
	        {
		    	...
		    	if(!qryHasElimtwoRecord.next()) //creates a new record for a person
		    	{
		    		...
		    		m_botAction.SQLQuery( mySQLHost, "INSERT INTO tblElimTwoStats"+num+"(fcUserName, fnPlayed, fnWon, fnKills, fnDeaths, fnRating) VALUES (\""+username+"\",1,"+winz+","+killz+","+losses+","+rating+")");
		    	}
		    	else //updates a persons record
		    	{
	                ...
	                double rating = (rating1 + rating2) * 500;
		    		m_botAction.SQLQuery( mySQLHost, "UPDATE tblElimTwoStats"+num+" SET fnPlayed = fnPlayed+1, fnWon = fnWon + "+winz+", fnKills = fnKills + "+killz+", fnDeaths = fnDeaths + "+ losses + ", fnRating = " +rating+ "  WHERE fcUserName = \"" + username+"\"");
