package twcore.bots.gamebot.scramble;
...
public class scramble extends MultiModule {
    ...
    String          m_prec = "-- ", gameType="Normal.";
    String          t_definition, t_word, s_word;
    boolean			difficulty = true, isVoting = false;
    String[]        helpmsg =
    { "Commands:",
      "!help          -- displays this.",
    ...
    public void doVote(){
		votes.clear();
		m_botAction.sendArenaMessage("Vote game type: 1 - Normal Mode, 2 - Nerd Mode");
		isVoting = true;
		TimerTask endVote = new TimerTask(){
			public void run(){
				...
				vote = countVote(2);
				if (vote == 1){
					if(!difficulty){doDifficulty(m_botAction.getBotName(),"");}
					gameType = "Normal Mode.";
				}
				else if(vote == 2){
					if(difficulty){doDifficulty(m_botAction.getBotName(),"");}
					gameType = "Nerd Mode.";
				...
				m_botAction.sendArenaMessage("Difficulty: " + gameType);
    ...
    public void registerCommands() {
        ...
        m_commandInterpreter.registerDefaultCommand( Message.PUBLIC_MESSAGE, this, "doCheckPublic" );
    }
    ...
    public void doCheckPublic( String name, String message) {
    	if (isVoting){
