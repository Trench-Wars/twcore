...
public class pictionary extends MultiModule {
	...
	String[] helpmsg = { "Commands:", "!help          -- displays this.",
	...
	String[] automsg = { "Commands:", "!help          -- displays this.",
			"!rules         -- displays the rules.",
			"!lagout        -- puts you back in the game if you're drawing.",
			"!pass          -- gives your drawing turn to a random player.",
			"!score         -- displays the current scores.",
	...
	String[] autoopmsg = { "Moderator Commands:",
			"!cancel                 -- Cancels this game of Pictionary.",
			"!showanswer             -- Shows you the answer."
	...
	String[] displayrules = {
			...
			"'    The first player to reach " + toWin
	...
	public void doNotPlaying(String name, String message) {
		if ((gameProgress == 3 || gameProgress == 2)
				&& name.equalsIgnoreCase(curArtist))
			doPass(curArtist, "");
		if (!notPlaying.contains(name)) {
			notPlaying.add(name);
			m_botAction
					.sendSmartPrivateMessage(name,
							"Not playing enabled. You will not be allowed to draw or guess.");
		} else {
			notPlaying.remove(name);
			m_botAction
					.sendSmartPrivateMessage(name,
							"Not playing disabled. You are now allowed to draw and guess.");
		}
	}
	...
	public void doHelp(String name, String message) {
		if (!autoStart)
			...
			m_botAction.smartPrivateMessageSpam(name, automsg);
	}
	...
	public void doRules(String name, String message) {
		if (m_botAction.getOperatorList().isModerator(name)) {
			if (!autoStart)
				...
				m_botAction.smartPrivateMessageSpam(name, autooprules);
		}
	...
	public void doStartGame(String name, String message) {
		if (m_botAction.getOperatorList().isModerator(name)
			...
			m_botAction.specAll();
			if (autoStart)
				doArenaRules(m_botAction.getBotName(), "");
			...
			if (m_botAction.getArenaSize() >= minPlayers) {
				if (!autoStart) {
					m_botAction
							.sendArenaMessage(
									m_prec
											+ "A game of Pictionary is starting | Win by getting "
											+ toWin + " pts!", 22);
					m_botAction.sendArenaMessage(m_prec
							+ "Type your guesses in public chat.");
					m_botAction.sendArenaMessage(m_prec + "PM !notplaying to "
				...
				m_botAction.sendArenaMessage(
						"There aren't enough players to play!", 13);
				int pNeed = minPlayers - m_botAction.getArenaSize();
				if (pNeed > 1) {
					m_botAction.sendArenaMessage(m_prec
							+ "Pictionary will begin when " + pNeed
							+ " more people enter.");
				} else {
					m_botAction.sendArenaMessage(m_prec
							+ "Pictionary will begin when " + pNeed
	...
	public void pickPlayer() {
		...
		if (m_botAction.getArenaSize() > minPlayers) {
			...
			if (!addPlayerName.equals(m_botAction.getBotName())
					&& !notPlaying.contains(addPlayerName))
	...
	public void displayHint() {
		timerHint = new TimerTask() {
			public void run() {
				if (gameProgress == 2) {
					gameProgress = 3;
					m_botAction.sendArenaMessage(m_prec + "Hint: "
							+ t_definition);
		...
		timerAnswer = new TimerTask() {
			public void run() {
				if (gameProgress == 3) {
					gameProgress = 4;
					m_botAction.sendArenaMessage(m_prec
							+ "No one has given the correct answer of '"
							+ t_word + "'", 103);
			...
			try {
				...
				if (qryWordData.next()) {
					...
					if (!(t_word.trim().split(" ").length > 1))
						t_definition = "The word begins with '"
	...
	public void doCustomWord(String name, String message) {
		if (name.equalsIgnoreCase(curArtist) && custGame && gameProgress == 1) {
			if (message.length() < 13) {
				//if(twcore.core.util.Spy.isRacist(message)){
				...
				//} 
				//else m_botAction.sendSmartPrivateMessage( curArtist, "Racist words are not allowed."); 
			}
			else
			...
			if (msg.contains(curAns)) {
				if (!cantPlay.contains(name) && !notPlaying.contains(name)) {
					...
					if (playerMap.containsKey(name)) {
						twcore.core.stats.PlayerProfile tempP = playerMap
								.get(name);
						...
						playerMap.put(name,
								new twcore.core.stats.PlayerProfile(name));
						twcore.core.stats.PlayerProfile tempP = playerMap
								.get(name);
						tempP.setData(0, 1);
						if (tempP.getData(0) >= toWin)
					...
					if (answerSpeed < 5) {
						...
						m_botAction.sendArenaMessage(m_prec + "Cheater! "
								+ name + " got the correct answer, '" + t_word
								+ "' in only " + trail, 13);
					} else if (answerSpeed < 25 && answerSpeed > 5) {
						String trail = getRank(tempP.getData(0));
						m_botAction.sendArenaMessage(m_prec + "Inconceivable! "
								+ name + " got the correct answer, '" + t_word
								+ "' in only " + trail, 7);
					} else {
						String trail = getRank(tempP.getData(0));
						m_botAction.sendArenaMessage(m_prec + name
								+ " got the correct answer, '" + t_word + "', "
								+ trail, 103);
					}
					if (gameProgress != -1) {
						gameProgress = 4;
					...
					if (name.equals(curArtist)) {
						m_botAction
								.sendSmartPrivateMessage(name,
										"A point has been deducted from your score for showing the answer.");
						if (playerMap.containsKey(curArtist)) {
							PlayerProfile tempPlayer = playerMap.get(curArtist);
							tempPlayer.decData(0);
						} else {
							playerMap.put(curArtist, new PlayerProfile(
									curArtist));
							PlayerProfile tempPlayer = playerMap.get(curArtist);
	...
	public void doCheckScores() {
		...
		if ((pictNumber % 5 == 0) && (curLeader != 0)) {
			...
			m_botAction.sendArenaMessage(m_prec2
					+ "-----------------------------|");
			m_botAction.sendArenaMessage(m_prec
					+ doTrimString("Top Scores", 28) + "|");
			m_botAction.sendArenaMessage(m_prec2
					+ "-----------------------------|");
			m_botAction.sendArenaMessage(m_prec
					+ doTrimString("Player Name", 20)
					+ doTrimString("Points", 8) + "|");
			while (numberShown < 5 && curPoints != 0) {
				...
				while (it.hasNext() && numberShown < 5) {
					...
					if (tempPlayer.getData(0) == curPoints) {
						numberShown++;
						m_botAction.sendArenaMessage(m_prec
								+ doTrimString(curPlayer, 20)
								+ doTrimString("" + tempPlayer.getData(0), 8)
								+ "|");
					}
				}
				curPoints--;
	...
	public void handleEvent(Message event) {
		...
		if (mType == Message.PUBLIC_MESSAGE
			...
			doCheckAnswers(m_botAction.getPlayerName(event.getPlayerID()),
					event.getMessage());
		} else {
	...
	public void getTopTen() {
		...
		try {
			...
			while (result != null && result.next())
				topTen.add(doTrimString(result.getString("fcUsername"), 17)
						+ "Games Won ("
						+ doTrimString("" + result.getInt("fnWon") + ":"
								+ result.getInt("fnPlayed") + ")", 9)
						+ "Pts Scored ("
						+ doTrimString("" + result.getInt("fnPoints") + ":"
								+ result.getInt("fnPossible") + ")", 10)
		...
		try {
			...
			if (!qryHasScrambleRecord.next()) {
				...
				m_botAction.SQLQueryAndClose(mySQLHost,
						"UPDATE tblPict_UserStats SET fnPlayed = fnPlayed+1, fnWon = fnWon + "
								+ wonAdd + ", fnPoints = fnPoints + " + points
								+ ", fnPossible = fnPossible + " + toWin
								+ ", fnRating = " + rating
	...
	public String getPlayerStats(String username) {
		try {
			...
			if (result.next())
				info = username + "- Games Won: (" + result.getInt("fnWon")
						+ ":" + result.getInt("fnPlayed") + ")  Pts Scored: ("
						+ result.getInt("fnPoints") + ":"
						+ result.getInt("fnPossible") + ")  Rating: "
						+ result.getInt("fnRating");
			m_botAction.SQLClose(result);
			return info;
		} catch (Exception e) {
			Tools.printStackTrace(e);
	...
	public void doScore(String name, String message) {
		...
		if (gameProgress != -1) {
			m_botAction.sendSmartPrivateMessage(name, "This game is to "
					+ toWin + " points.");
			m_botAction.sendSmartPrivateMessage(name, m_prec
					+ "----------------------------");
			m_botAction.sendSmartPrivateMessage(name, m_prec
					+ doTrimString("Current Scores", 28) + "|");
			m_botAction.sendSmartPrivateMessage(name, m_prec
					+ doTrimString("Player Name", 18)
					+ doTrimString("Points", 10) + "|");
			...
			while (curPoints != 0) {
				...
				while (it.hasNext()) {
					...
					if (tempPlayer.getData(0) == curPoints) {
						m_botAction.sendSmartPrivateMessage(name, m_prec
								+ doTrimString(curPlayer, 18)
								+ doTrimString("" + tempPlayer.getData(0), 10)
								+ "|");
					}
				}
				curPoints--;
	...
	public void doMyScore(String name, String message) {
		if (gameProgress != -1) {
			if (playerMap.containsKey(name)) {
				...
				m_botAction.sendSmartPrivateMessage(name, "You currently have "
						+ temp.getData(0) + " points.");
			} else {
				m_botAction.sendSmartPrivateMessage(name,
	...
	public void doEndGame(String name) {
		...
		m_botAction.sendArenaMessage(m_prec + "Player: " + name
				+ " has won this round of Pictionary!", 5);
		// Save statistics
		Iterator<String> it = playerMap.keySet().iterator();
		while (it.hasNext()) {
			String curPlayer = (String) it.next();
