 ...
 */
public final class radiobot2 extends SubspaceBot {
    ...
    public radiobot2(BotAction botAction) {
        super(botAction);
    ...
    public void handleEvent(LoggedOn event) {
        ...
        m_botAction.sendUnfilteredPublicMessage("?chat=" + "ECT");
       }
    
    public void handleEvent( ArenaJoined event ){
        m_botAction.sendChatMessage(m_botAction.getBotName() + " is now online. By Dezmond.");
        m_botAction.sendChatMessage("Rules of hosting is located at twradio.webs.com. Also watch out for HMS being around :P.");
    }
    ...
    public void handleEvent(PlayerEntered event) {
		...
		boolean isMod = m_opList.isModerator(name);
		 ...
		 * Handle Mod+ commands
		 */
		if(isMod && handleModMessage(name, id, message)) {
			return;
		}
    ...
    private boolean handleStaffMessage(String name, int id, String message) {
		...
		if(message.startsWith("!host")) {
            ...
            if(!m_someoneHosting){
                ...
                m_botAction.sendPrivateMessage(id, "Someone is already hosting. Wait until they are done, or get a mod+ to !unhost them first.");
            }
            ...
            m_botAction.sendPrivateMessage(id, "Radiobot Update By Dezmond");
    ...
    private boolean handleCurrentHostOnly(String name, int id, String message, int sound) {
		...
		if(message.startsWith("!arena ") && name.equals(m_currentHost)) {
            ...
            if(m_alreadyZoned.contains(name)) {
                m_botAction.sendPrivateMessage(id, "Sorry, you've already used your zone message. Get a moderator to grant you another.");
    ...
    private boolean handlePrivateMessage(String name, int id, String message) {
		...
		if(!m_someoneHosting
    		...
    		handled = true;
    		m_botAction.sendSmartPrivateMessage(name, "Trenchwars Radio is managed by a Radio Server.We use Winamp and Shoutcast to feed music to our listeners. Think you could do this ? fill out the application at http://twradio.webs.com/application.htm");
