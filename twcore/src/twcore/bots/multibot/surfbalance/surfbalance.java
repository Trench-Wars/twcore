 ...
 * Created: 12/2012
 * @author Fusha
 * With stuff stolen from: 
 * @author K A N E (Ian)
 * and
 * @author qan
 * 
 * This module is for doing a racing event in ?go surfbalance
 * When a player completes part 1, they get assigned to part 2, etc.
 * First player to get to the end wins.
 * 
...
public class surfbalance extends MultiModule {
    ...
    private void cmd_enter(String name) {
        if (isRunning) {
            ...
            else m_botAction.sendPrivateMessage(name, "Please Use The !lagout Command!");
        } else m_botAction.sendPrivateMessage(name, "The game is not currently started.");
    }
    ...
    private void cmd_backwards(String name) {
    	...
    	} else m_botAction.sendPrivateMessage(name, "Can't switch game modes when game is started!");
    }
    ...
    /**
     * Handles player position and switces freqs.
     */
    ...
    private void announceChange(String playerName, int freq) {
    	String time;
    	if (!isRunning) return; //NullPointerException error otherwise
    	time = checkTimer.getPlayTimeString();
    	if (freq == 1) {
    		if (!backwards)
        	m_botAction.sendArenaMessage(playerName + " has reached part 2! Time: " + time, 20);
    		else m_botAction.sendArenaMessage(playerName + " has reached part 3! Time: " + time, 21);
    	}
    	if (freq == 2) {
    		if (!backwards)
    		m_botAction.sendArenaMessage(playerName + " has reached part 3! Time: " + time, 21);
    		else m_botAction.sendArenaMessage(playerName + " has reached part 2! Time: " + time, 20);
    	}
    	if (freq == 3 || freq == 0)
    ...
    private class CheckTimer extends TimerTask {
        ...
        private void doWarning() {
            ...
            } else m_botAction.setAlltoFreq(3);	
        }
        ...
        private void doGo() {
            ...
            if (!backwards) m_botAction.setDoors(0);
            else m_botAction.warpAllRandomly();
            isRunning = true;
            createPlayerRecords();
    ...
    public String[] getModHelpMessage() {
        ...
        String[] JBHelp = { 
                "!start           -- Locks the arena and starts the game!",
                "!backwards       -- Toggles racing from finish to start.",
