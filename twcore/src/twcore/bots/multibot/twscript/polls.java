...
public class polls extends MultiUtil {
	...
	public void do_vote(String name, String message){
		...
		if (polls.get(msg[0]).twdOnly && !isTWDRegistered(name)) {
			m_botAction.sendSmartPrivateMessage( name, "Sorry! Only TWD registered players are allowed to vote on this poll.");
		} else {
	...
	public void do_setPoll(String name, String message){
		...
		if(!polls.containsKey(msgs[0])){
			m_botAction.sendSmartPrivateMessage( name, "Poll '" + msgs[0] + "' not found.");
			return;
		}
		if(msgs[1].equalsIgnoreCase("true")) {
			polls.get(msgs[0]).twdOnly = true;
			m_botAction.sendSmartPrivateMessage( name, "Poll '" + msgs[0] + "' is now restricted to TWD Registered players.");
		} else if (msgs[1].equalsIgnoreCase("false")) {
			polls.get(msgs[0]).twdOnly = false;
			m_botAction.sendSmartPrivateMessage( name, "Poll '" + msgs[0] + "' is not restricted to TWD Registered players.");
		} else
			m_botAction.sendSmartPrivateMessage( name, "Incorrect Usage. For example, to exclude poll to TWD registered players only use: !setpoll favcolor:true");
	}
