 ...
 * 1.08
 * Upgrades:
 * 		- gamebot trashed. Module moved to multibot. -milosh 2.21.08
 */
public class boggle extends MultiModule {
	...
	TreeMap<String, BogglePlayer> playerMap = new TreeMap<String, BogglePlayer>();
 ...
 * @param bp - The BogglePlayer object
 * @param wonRound - Whether or not the player won the current round.
 * @param wonGame - Whether or not the player won the current game.
 * @param s - Whether or not the current game is ending.
 */
...
public void doStats(String name, String message) {
	if (gameProgress == NO_GAME_IN_PROGRESS) {
		...
		m_botAction.sendSmartPrivateMessage(name,
				"Please check your stats when Boggle is not running.");
	}
}
...
public String getPlayerStats(String username) {
	try {
		...
		ResultSet result = m_botAction.SQLQuery(
						mySQLHost,
						"SELECT * FROM tblBoggle_UserStats WHERE fcUserName = \""
								+ username + "\"");
		String info = "There is no record of player '" + username + "'. Please be sure you are using the entire name.";
		if (result.next())
			info = username + "- Games Won: (" + result.getInt("fnGamesWon")
					+ ":" + result.getInt("fnGamesPlayed") + ")  Rounds Won: ("
					+ result.getInt("fnRoundsWon") + ":"
					+ result.getInt("fnRoundsPlayed") + ")  Average Points: "
					+ result.getInt("fnAveragePoints") + " Rating: "
					+ result.getInt("fnAverageRating");
...
public void getTopTen() {
	...
	try {
		ResultSet result = m_botAction.SQLQuery(
						mySQLHost,
						"SELECT * FROM tblBoggle_UserStats ORDER BY fnAverageRating DESC LIMIT 10");
		while (result.next())
			topTen.add(result.getString("fcUserName") + "- Games Won: (" + result.getInt("fnGamesWon")
					+ ":" + result.getInt("fnGamesPlayed") + ")  Rounds Won: ("
					+ result.getInt("fnRoundsWon") + ":"
					+ result.getInt("fnRoundsPlayed") + ")  Average Points: "
					+ result.getInt("fnAveragePoints") + " Rating: "
