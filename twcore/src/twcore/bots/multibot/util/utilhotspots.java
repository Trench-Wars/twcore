...
public class utilhotspots extends MultiUtil {
    ...
    public void do_addHotSpot(String sender, String message) {
        ...
        if (index == -1) {
            ...
            if (values.length >= 5)
                hotSpots.get(index).messages.add("*warpto " + values[3] + " " + values[4]);
            if (values.length == 6)
                hotSpots.get(index).messages.add("*prize #" + values[5]);
    ...
    public void do_addMessage(String sender, String message) {
        ...
        hotSpots.get(index).messages.add(msg);
    ...
    public void do_removeMessage(String sender, String message) {
        ...
        try{
        	hotSpots.get(values[0]).messages.remove(values[1]);
    ...
    public void do_listMessages(String sender, String message) {
        ...
        if (hotSpots.get(index).messages == null) {
            m_botAction.sendPrivateMessage(sender, "There are no messages assigned to this spot.");
        } else {
            int i = 0;
            Iterator<String> it = hotSpots.get(index).messages.iterator();
            while (it.hasNext()) {
                String msg = it.next();
            ...
            if (watch.inside(x, y)) {
                ...
                if (watch.messages != null) {
                    ...
                    while( i.hasNext() ) {
                        ...
                        m_botAction.sendUnfilteredPrivateMessage(p.getPlayerName(), i.next());
                    }
                }
            }
        }
