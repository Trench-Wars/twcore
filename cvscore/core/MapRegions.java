...
public class MapRegions
{
	...
	public BotSettings loadRegionCfg(String filename) throws IOException
	{
		...
		for(int x = 0; x < regions; x++)
		{
			...
			setRegion(x,r,g,b);
			m_botAction.sendChatMessage("Loaded region "+ x +": "+ m_regions[x]);
		}
	...
	{
		int mapColor = m_map.getRGB(x, y);// & 0xffffff;
		int regColor = m_regions[region];// & 0xffffff;
		System.out.println(mapColor +","+ regColor);
	...
	public void setRegion(int region, int rgb)
	{
		if(region < m_regions.length && region > -1)
			...
			System.out.println("Regions: bad region index");
	}
	...
	/** Simple test to determine if the map exists
	 * @return true if m_map is null, otherwise false
	 */
	...
	//testMap wrapper for printing diagnostic info to console
	public void testMap(String point)
	{
		if(testMap())
			Tools.printLog("map failed at "+ point);
	}
}